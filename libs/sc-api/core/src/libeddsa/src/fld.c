	...
	for (i = 0; i < FLD_LIMB_NUM; i++) {
		for (;fill < FLD_LIMB_BITS; fill += 8)
			tmp |= (uint64_t)*src++ << fill;
                ...
                dst[i] = (limb_t)(tmp & FLD_LIMB_MASK);
                tmp >>= FLD_LIMB_BITS;
                fill -= FLD_LIMB_BITS;
        }
        dst[0] += (limb_t)(19 * tmp);
...
fld_scale(fld_t res, const fld_t e, limb_t s)
{
	...
	carry = (llimb_t)s*e[0];
        res[0] = (limb_t)(carry & FLD_LIMB_MASK);

        carry  = (carry >> FLD_LIMB_BITS) + (llimb_t)s * e[1];
        res[1] = (limb_t)(carry & FLD_LIMB_MASK);

        carry  = (carry >> FLD_LIMB_BITS) + (llimb_t)s * e[2];
        res[2] = (limb_t)(carry & FLD_LIMB_MASK);

        carry  = (carry >> FLD_LIMB_BITS) + (llimb_t)s * e[3];
        res[3] = (limb_t)(carry & FLD_LIMB_MASK);

        carry  = (carry >> FLD_LIMB_BITS) + (llimb_t)s * e[4];
        res[4] = (limb_t)(carry & FLD_LIMB_MASK);

        res[0] += (limb_t)(19 * (carry >> FLD_LIMB_BITS));
}
...
fld_mul(fld_t res, const fld_t a, const fld_t b)
{
	...
	c[4] += c[3] >> FLD_LIMB_BITS;
        ...
        tmp    = (limb_t)((c[0] & FLD_LIMB_MASK) + 19 * (c[4] >> FLD_LIMB_BITS));
        res[1] = (limb_t)((c[1] & FLD_LIMB_MASK) + (tmp >> FLD_LIMB_BITS));

        res[0] = (limb_t)(tmp & FLD_LIMB_MASK);
        res[2] = (limb_t)(c[2] & FLD_LIMB_MASK);
        res[3] = (limb_t)(c[3] & FLD_LIMB_MASK);
        res[4] = (limb_t)(c[4] & FLD_LIMB_MASK);
}
	...
	for (i = 0; i < FLD_LIMB_NUM; i++) {
		for (; fill < FLD_LIMB_BITS(d); fill += 8)
			foo |= (uint32_t)*src++ << fill;
                dst[i] = (limb_t)(foo & FLD_LIMB_MASK(d));
...
fld_scale(fld_t dst, const fld_t e, limb_t x)
{
	...
	for (tmp = 0, i = 0; i < FLD_LIMB_NUM; i += 2) {
		tmp = (tmp >> FLD_LIMB_BITS(0)) + (llimb_t) x*e[i];
                dst[i]     = (limb_t)(tmp & FLD_LIMB_MASK(1));
                tmp        = (tmp >> FLD_LIMB_BITS(1)) + ((llimb_t)x * e[i + 1]);
                dst[i + 1] = (limb_t)(tmp & FLD_LIMB_MASK(0));
        }
        dst[0] += (limb_t)(19 * (tmp >> FLD_LIMB_BITS(0)));
}
...
fld_sq(fld_t dst, const fld_t a)
{
	...
	c[9] = 2*((llimb_t)a[0]*a[9] + (llimb_t)a[1]*a[8] + (llimb_t)a[2]*a[7] + (llimb_t)a[3]*a[6] + (llimb_t)a[4]*a[5]);
        ...
        c[0] += (llimb_t)19 * 2 *
                ((llimb_t)2 * a[1] * a[9] + (llimb_t)a[2] * a[8] + (llimb_t)2 * a[3] * a[7] + (llimb_t)a[4] * a[6] +
                 (llimb_t)a[5] * a[5]);
        c[1] += (llimb_t)19 * 2 *
                ((llimb_t)a[2] * a[9] + (llimb_t)a[3] * a[8] + (llimb_t)a[4] * a[7] + (llimb_t)a[5] * a[6]);
        c[2] += 19 * (2 * ((llimb_t)2 * a[3] * a[9] + (llimb_t)a[4] * a[8] + (llimb_t)2 * a[5] * a[7]) +
                      (llimb_t)a[6] * a[6]);
        c[3] += (llimb_t)19 * 2 * ((llimb_t)a[4] * a[9] + (llimb_t)a[5] * a[8] + (llimb_t)a[6] * a[7]);
        c[4] += 19 * (2 * ((llimb_t)2 * a[5] * a[9] + (llimb_t)a[6] * a[8] + (llimb_t)a[7] * a[7]));
        c[5] += (llimb_t)19 * 2 * ((llimb_t)a[6] * a[9] + (llimb_t)a[7] * a[8]);
        c[6] += 19 * ((llimb_t)2 * 2 * a[7] * a[9] + (llimb_t)a[8] * a[8]);
        c[7] += (llimb_t)19 * 2 * (llimb_t)a[8] * a[9];
        c[8] += (llimb_t)19 * 2 * (llimb_t)a[9] * a[9];
