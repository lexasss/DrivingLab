...
const sc_t con_off = {1530200761952544, 2593802592017535, 2401919790321849, 2401919801264264, 9382499223688};
...
/* off = 8 * (16^64 - 1) / 15 mod m */
const sc_t con_off = {14280992, 22801768, 35478655, 38650670, 65114297, 35791393, 8947848, 35791394, 8947848, 139810};
	...
	for (j = K+1; j <= 2*K; j++) {
		...
		for (i = j-K; i <= K; i++)
			carry += (llimb_t)x[K-1+i] * con_mu[j-i];
                ...
                q[j - K - 1] = (limb_t)(carry & MSK);
        }
        q[j - K - 1] = (limb_t)(carry >> LB);
        ...
        for (j = 0, carry = 0; j <= K; j++) {
            ...
            for (i = 0; i <= j; i++) carry += (llimb_t)q[i] * con_m[j - i];

            r[j] = (limb_t)(carry & MSK);
        }

        /* r <- x - r mod b^(k+1) */
        for (i = 0, carry = 0; i <= K; i++) {
            carry = (carry >> LB) + x[i] - r[i];
            r[i]  = (limb_t)(carry & MSK);
        }
...
void sc_import(sc_t dst, const uint8_t* src, size_t len) {
    ...
    for (i = 0; i < 2 * K; i++) {
        while (src < endp && fill < LB) {
            // fill can never be negative as it only becomes negative when src >= endp when this is exited early and we
            // won't come inside this again
            // NOLINTNEXTLINE
            foo |= (uint64_t)*src++ << fill;
            fill += 8;
        }

        tmp[i] = (limb_t)(foo & MSK);
	...
	for (k = 0; k < K; k++) {
		...
		for (i = 0; i <= k; i++)
			carry += (llimb_t) a[i] * b[k-i];
                tmp[k] = (limb_t)(carry & MSK);
        }

        for (k = K; k < 2 * K - 1; k++) {
            carry >>= LB;
            for (i = k - K + 1; i <= K - 1; i++) carry += (llimb_t)a[i] * b[k - i];
            tmp[k] = (limb_t)(carry & MSK);
        }
        tmp[k] = (limb_t)(carry >> LB);
...
jsfdigit(unsigned int a, unsigned int b)
{
    int u = (int)(2 - (a & 0x03));
    if (u == 2) return 0;
    if (((a & 0x07) == 3 || (a & 0x07) == 5) && (b & 0x03) == 2) return -u;
    return u;
}
