...
// NOLINTBEGIN(bugprone-assignment-in-if-condition,clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)
 ...
 *
 ******************************************************************************/
int SC_API_gcm_initialize(void) {
    SC_API_aes_init_keygen_tables();
    return( 0 );
}
 ...
 *
 ******************************************************************************/
static void SC_API_gcm_mult(gcm_context* ctx,    // pointer to established context
 ...
 *
 ******************************************************************************/
int SC_API_gcm_setkey(gcm_context* ctx,    // pointer to caller-provided gcm context
                                           ...
                                           // 128, 192 or 256-bit keys respectively)
{
    ...
    if ((ret = SC_API_aes_setkey(&ctx->aes_ctx, ENCRYPT, key, keysize)) != 0) return (ret);
    if ((ret = SC_API_aes_cipher(&ctx->aes_ctx, h, h)) != 0) return (ret);
 ...
 *
 ******************************************************************************/
int SC_API_gcm_start(gcm_context* ctx,     // pointer to user-provided GCM context
                     ...
                     size_t       add_len)       // length of additional AEAD data (bytes)
{
    ...
    {   
        ...
        while( iv_len > 0 ) {
            ...
            SC_API_gcm_mult(ctx, ctx->y, ctx->y);
            iv_len -= use_len;
            p += use_len;
        }
    ...
    if ((ret = SC_API_aes_cipher(&ctx->aes_ctx, ctx->y, ctx->base_ectr)) != 0) return (ret);
    ...
    while( add_len > 0 ) {
        ...
        SC_API_gcm_mult(ctx, ctx->buf, ctx->buf);
        add_len -= use_len;
        p += use_len;
    }
    return( 0 );
 ...
 *
 ******************************************************************************/
int SC_API_gcm_update(gcm_context* ctx,     // pointer to user-provided GCM context
                      ...
                      uchar*       output)        // pointer to destination data
{
    ...
    while( length > 0 ) {
        ...
        if ((ret = SC_API_aes_cipher(&ctx->aes_ctx, ctx->y, ectr)) != 0) return (ret);
        ...
        SC_API_gcm_mult(ctx, ctx->buf, ctx->buf);  // perform a GHASH operation
 ...
 *
 ******************************************************************************/
int SC_API_gcm_finish(gcm_context* ctx,  // pointer to user-provided GCM context
 ...
 *
 ******************************************************************************/
int SC_API_gcm_crypt_and_tag(gcm_context* ctx,      // gcm context with key already setup
...
{                                                   /*
                                                    ...
                                                    */
    SC_API_gcm_start(ctx, mode, iv, iv_len, add, add_len);
    SC_API_gcm_update(ctx, length, input, output);
    SC_API_gcm_finish(ctx, tag, tag_len);
    return( 0 );
}
 ...
 *
 ******************************************************************************/
int SC_API_gcm_auth_decrypt(gcm_context* ctx,      // gcm context with key already setup
                            const uchar* iv,       // pointer to the 12-byte initialization vector
                            size_t       iv_len,   // byte length if the IV. should always be 12
                            const uchar* add,      // pointer to the non-ciphered additional data
                            size_t       add_len,  // byte length of the additional AEAD data
                            ...
                            size_t       tag_len)        // byte length of the tag <= 16
{
    ...
    */
    SC_API_gcm_crypt_and_tag(ctx, DECRYPT, iv, iv_len, add, add_len, input, output, length, check_tag, tag_len);
 ...
 *
 ******************************************************************************/
void SC_API_gcm_zero_ctx(gcm_context* ctx) {
    // zero the context originally provided to us
    memset( ctx, 0, sizeof( gcm_context ) );
}

// NOLINTEND(bugprone-assignment-in-if-condition,clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)
